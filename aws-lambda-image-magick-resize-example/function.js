...
//
const imageBucketName = "serverless-s3-bucket-ting"

// main function
module.exports.invoke = async () => {
  await downloadFile(s3Key, localFile);
  await resizeImage(localFile);
  await uploadThumbnailToS3(localFile, s3Key);
...
async function uploadThumbnailToS3(localFilePath, s3Key) {
  const fileBuffer = await fs.promises.readFile(localFilePath);
  
  await s3.putObject({
    Bucket: imageBucketName,
    Key: s3Key + "-thumbnail.jpg",
    Body: fileBuffer,
